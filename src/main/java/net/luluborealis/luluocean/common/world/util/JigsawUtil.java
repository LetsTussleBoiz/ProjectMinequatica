...
import net.luluborealis.luluocean.mixin.access.StructureTemplatePoolAccess;
import net.luluborealis.luluocean.util.GSONUtil;
...
public class JigsawUtil {
    ...
    private static void addBuildingToPool(ResourceLocation poolRL, String nbtPieceRL, int weight,
        ...
        if (pool == null) {
            LuluOcean.logWarning(String.format("Ignoring pool \"%s\" bc it's not available.", poolRL.toString()));
            return;
        }
        ...
        for (Pair<StructurePoolElement, Integer> listOfPieceEntry : listOfPieceEntries) {
            ...
            if (nbtPieceRL.equals(location)) {
                LuluOcean.logInfo(String.format("Not appending structure pool element \"%s\" because it is already specified via data packs for this pool \"%s\".", nbtPieceRL, poolRL.toString()));
                return;
            }
        }
